...
`./s/start_jupyter.sh`命令，然后复制URL到浏览器地址栏访问，最后在浏览器中去使用.
...
## 三原色
一幅**图像**你可以把他想象成一个**二维矩阵**, 如果再把**颜色**也考虑进来，我们
完全可以把**颜色数据**作为另一个空间纬度，自然就容易把这个图像又想象成一个**三
维矩阵**

如果我们选择使用[三原色（红、绿、蓝）
](https://en.wikipedia.org/wiki/Primary_color)来描述这些颜色，我们就定义了三个
平面：第一个是**红色**，第二个是**绿色**，最后一个是**蓝色**。

![an image is a 3d matrix RGB](/i/image_3d_matrix_rgb.png "An image is a 3D matrix")

## 像素
这个矩阵里的每一个点, 我们称为**像素**(图像元素）。每个像素是由不同强度(通常是
数值表示）的三原色(RGB)光组成）。例如，**红色像素**是由强度为 0的绿色，强度为0
的蓝色和强度最大的红色组成。**粉色像素**也可以通过三原色的组合表示。如果规定强
度的取值范围是 0 到 255，粉色可以定义为**红色 255、绿色 192、蓝色 203**

> #### 编码彩色图像的其它方法还有许多其它可行模型也可以用来表示组成一幅图像的
> 颜色。例如，使用索引调色板，我们就可以不用使用 RGB 模型3个数值，而是一个单个
> 数值来表示颜色。 在这个模型下，我们可以用一个二维矩阵来代替三维矩阵去
> 表示我们的颜色，这将节省存储空间，但颜色选择会更少点。(译注: 位数够就不是问
> 题）
>
> ![NES palette](/i/nes-color-palette.png "NES palette")


例如以下几张图片。第一脸包含所有颜色。其他几个分别是红、绿、蓝色平面（显示为灰
调）（译注：如强度0为黑色，强度255为白色）。


![RGB channels intensity](/i/rgb_channels_intensity.png "RGB channels intensity")

我们可以看到，对于最终的成像，**红色颜色**对像素贡献更多（第二张脸最亮的部分）
，**蓝色颜色**的贡献大多只在马里奥的眼睛(最后那张脸）和他衣服的一部分。所有颜
色平面对**马里奥的胡子**（最暗的部分）都贡献最少。

## 位深
每个颜色的强度，采用一定数量的二进制位数来存储，位数大小被称为颜色深度。假如每
个颜色（平面）的强度占用 8 bit（取值范围为 0 到 255），那么颜色深度就是 24（
8x3）位，我们还可以推导出我们可以使用 2 的 24 次方种不同的颜色。

> 很棒的学习材料：[现实世界的照片是如何拍摄成 0 和 1 的
> ](http://www.cambridgeincolour.com/tutorials/camera-sensors.htm)。

## 分辨率
图像的另一个属性是**分辨率**，即一个平面内像素的数量。通常表示成宽 x 高，例如
下面这张 **4x4** 的图像。
...
> ### 自己动手：玩转图像和颜色
>
> 你可以使用 [jupyter](#如何使用-jupyter)（python, numpy, matplotlib 等等）[玩
> 转图像](/image_as_3d_array.ipynb)。
>
> 你也可以学习[图像滤镜（边缘检测，锐化，模糊..）的工作原理
> ](/filters_are_easy.ipynb)。

## 宽高比
图像或视频还有一个工作属性就是**宽高比**，它简单地描述了图像或像素的宽度和高度
之间的一个比例关系。

当人们说这个电影或图像是 **16:9** 时，通常是指**显示纵横比（DAR)**，但我们也会遇到
...
> ### DVD 4:3 显示纵横比 
>
> DVD 的实际分辨率是 704x480，但为何一般都说他的纵横比时 4:3， 原因是它的像素
> 纵横比是10:11（704x10／480x11)。


##  帧率
**视频**可以定义为在**单位时间**内**连续的 n 帧**，其中时间可以视为另一个维度
，n 即为帧率，或每秒帧数(FPS)。

![video](/i/video.png "video")

## 比特率
播放一段视频每秒所需的比特数就是它的**比特率**（即常说的码率）。
> 原始比特率 = 宽 * 高 * 位深 * 帧率 

例如，一段帧率为30，位深为24，分辨率为480x240的视频，如果是原始未压缩数据，它
将需要 **82,944,000 比特每秒** 或 82.944 Mbps (30x480x240x24)。

当**比特率**接近恒定时称为恒定比特率（**CBR**）；但如果是波动的，则称为可变比
特率（**VBR**）。

> 下面图形显示了一个限制最大码率和最小码率的压缩码率的VBR，可以看到当帧为黑色
> 数据量最小。
>
> ![constrained vbr](/i/vbr.png "constrained vbr")

## 隔行扫描 
在早期，为了节约传输带宽，工程师们发明了一项技术，可以将视频的感官帧率加倍而**
没有消耗额外带宽**。这项技术就是大家都熟知的**隔行扫描**.

隔行扫描的一帧图像由2部分组成，一部分由奇数行组成，称为奇数场，另一部分就是偶
数行组成，称为偶数行。在隔行扫描中，电子束扫完第1行后回到第3行开始的位置接着扫
，然后在第5、7、...，行上扫，直到最后一行。奇数行扫完后接着扫偶数行，这样就完
成了一帧的完整扫描。

## 逐行扫描
一般黑白电视和老的彩色电视都采用隔行扫描， 而计算机显示器, 手机，pad, 新的
4K电视都使用逐行扫描显示，在逐行扫描中，电子束从显示屏的左上角一行接一行地扫到
右下角，在显示屏上扫一遍就显示一幅完整的图像。

![interlaced vs progressive](/i/interlaced_vs_progressive.png "interlaced vs progressive")

## 小结
现在我们知道了数字化**图像**的原理；它的**颜色**的编排方式；给定**帧率**和**分
辨率**时，展示一个视频需要花费多少**比特率**；它是恒定的（CBR）还是可变的（VBR
）；还有很多其它内容，如隔行扫描和 PAR。
